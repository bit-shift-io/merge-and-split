...
impl Material {
    pub fn from_file(file_name: String, device: &wgpu::Device, queue: &wgpu::Queue) -> Self {
        use std::fs;
        ...
        let diffuse_texture =
            texture::Texture::from_bytes(&device, &queue, &diffuse_bytes, &file_name).unwrap();

        // This appears to be seperate from the material, refer: https://github.com/sotrh/learn-wgpu/blob/master/code/beginner/tutorial9-models/src/lib.rs
        // and should be passed in.
...
impl Mesh {
    pub fn from_verticies_and_indicies<A: NoUninit, B: NoUninit>(name: String, device: &wgpu::Device, vertices: &[A], indices: &[B]) -> Self {
    ...
    pub fn render(&self, render_pass: &mut wgpu::RenderPass,/* , material: &Material, camera_bind_group: &wgpu::BindGroup, */instances: std::ops::Range<u32>) {
        render_pass.set_vertex_buffer(0, self.vertex_buffer.slice(..));
        render_pass.set_index_buffer(self.index_buffer.slice(..), wgpu::IndexFormat::Uint16); //Uint32);
...
//         let diffuse_bytes = include_bytes!("marble.png");
//         let diffuse_texture =
