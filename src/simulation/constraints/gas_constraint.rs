...
pub struct GasConstraintVec(pub Vec<GasConstraint>);

impl GasConstraintVec {
    pub fn new() -> Self {
        Self(vec![])
    }
    ...
    pub fn push(&mut self, c: GasConstraint) {
        self.0.push(c);
    }
    ...
    pub fn iter_mut(&mut self) -> std::slice::IterMut<'_, GasConstraint> {
        self.0.iter_mut()
    }
...
impl std::ops::Index<usize> for GasConstraintVec {
    type Output = GasConstraint;
...
impl std::ops::IndexMut<usize> for GasConstraintVec {
