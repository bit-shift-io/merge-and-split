...
use crate::{particle::Particle, particle_system::ParticleSystem};



pub struct ParticleSystemMerge {
}

impl ParticleSystemMerge {
    fn execute(&self, ps: &mut ParticleSystem) {
        ...
        for ai in 0..particle_count {
            for bi in (&ai+1)..particle_count {
                ...
                let delta_e = ((p1.mass * p2.mass) / (2.0 * m12)) * (p1.vel - p2.vel).magnitude2();
...
impl Default for ParticleSystemMerge {
    fn default() -> Self {
        Self {
        }
    }
...
mod tests {
    ...
    fn execute() {
        ...
        let psm = ParticleSystemMerge::default();
        psm.execute(&mut ps);
